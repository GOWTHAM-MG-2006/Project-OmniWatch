...
Design decisions:
  ...
  record re-arrival (the loader is idempotent).
   ...
   Live-only: the graph is populated exclusively from the
   omniwatch.entities.resolved Kafka topic. An empty topic means an empty
   graph — no built-in seed data is loaded.
 """
...
class TopologyLoader:
    ...
    # ------------------------------------------------------------------ #
    # CLI entrypoint (live Kafka consumer only)
    # ------------------------------------------------------------------ #
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        "omniwatch.entities.resolved and materializes the live "
        "dependency graph into Neo4j.",
    )
    parser.parse_args()
