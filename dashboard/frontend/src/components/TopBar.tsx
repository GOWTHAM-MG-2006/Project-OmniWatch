...
import { clearSession, getSession, hasActiveWorkspace, SESSION_INVALID_EVENT } from '../auth/session'
import { identityLogout } from '../api/client'
import { useEffect } from 'react'

interface TopBarProps {
  /** When false, hide workspace-dependent controls (picker, copilot, switcher).
   *  Set by the parent shell: EntryShell passes false, DashboardShell uses default true. */
  showWorkspaceControls?: boolean
}

export function TopBar({ showWorkspaceControls = true }: TopBarProps) {
  ...
  return (
    <>
      <header className="h-12 bg-bg-card border-b border-border-default flex items-center px-4 gap-4 shrink-0">
        ...
        {/* Timeframe Picker — writes to URL ?timeRange= (dashboard only) */}
        {showWorkspaceControls && hasActiveWorkspace() && (
        ...
        {showWorkspaceControls && authed && <WorkspaceSwitcher />}
        ...
        {/* Copilot Button (dashboard only — requires active workspace) */}
        {showWorkspaceControls && hasActiveWorkspace() && (
          <button
      ...
      {showWorkspaceControls && hasActiveWorkspace() && <CopilotDrawer open={copilotOpen} onClose={() => setCopilotOpen(false)} />}
    </>
  )
}
